...
5. Corre `pnpm test:complexity` sobre los archivos tocados (umbral inicial 6). Si una función lo excede, refactorízala para bajar su complejidad ciclomática — a diferencia del mutation score, este sí es un número que puede fallar la etapa. Si 6 resulta demasiado agresivo para el código real de la historia, sube el umbral a 8 en el comando (edita el `--max` de `test:complexity` para esta corrida) antes de insistir con refactors forzados.
